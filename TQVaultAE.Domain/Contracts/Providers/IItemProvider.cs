...
public interface IItemProvider
{
	...
	ToFriendlyNameResult GetFriendlyNames(Item item, FriendlyNamesExtraScopes? scopes = null, bool filterExtra = true);

	/// <summary>
	/// Shows the items in a set for the set items
	/// </summary>
	/// <returns>string containing the set items</returns>
	SetItemInfo GetItemSetTranslations(Item item);
	...
	int GetPetSkillLevel(Item item, DBRecordCollection record, RecordId recordId, int varNum);
	...
	SortedList<string, Variable> GetRequirementVariables(Item item);
	...
	SetItemInfo GetSetItems(Item item);
	...
	int GetTriggeredSkillLevel(Item item, DBRecordCollection record, RecordId recordId, int varNum);

	/// <summary>
	/// Parses an item from the save file format
	/// </summary>
	/// <param name="reader">BinaryReader instance</param>
	void Parse(Item item, BinaryReader reader);

	/// <summary>
	/// Removes the first relic from this item
	/// </summary>
	/// <returns>Returns the removed relic as a new Item</returns>
	Item RemoveRelic1(Item item);

	/// <summary>
	/// Removes the second relic from this item
	/// </summary>
	/// <returns>Returns the removed relic as a new Item</returns>
	Item RemoveRelic2(Item item);
