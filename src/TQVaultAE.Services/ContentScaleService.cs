...
{
	public class ContentScaleService : IContentScaleService, IWindowSizeObserver
	{
		...
		private static ContentScaleService? s_instance;
		private static readonly object s_instanceLock = new();

		private readonly WindowSizeService _windowSizeUpdater;
		private readonly List<IContentScaleObserver> _observers = [];

		public static ContentScaleService GetInstance()
		{
			if (s_instance is null)
			{
				lock (s_instanceLock)
				{
					s_instance ??= new ContentScaleService();
				}
			}
		...
		private ContentScaleService() 
		{ 
			_windowSizeUpdater = WindowSizeService.GetInstance();
