...
{
    public class TitanQuestDataService : ITitanQuestDataService
    {
        ...
        public bool MatchNextString(string value, BinaryReader reader)
        {
            ...
            return label.ToUpperInvariant().Equals(value.ToUpperInvariant());
        }
        ...
        public void WriteCString(BinaryWriter writer, string value)
        {
            ...
            Encoding asciiEncoding = Encoding.GetEncoding(1252);
            byte[] rawstring = asciiEncoding.GetBytes(value);
        ...
        public string ReadCString(BinaryReader reader)
        {
            ...
            char[] chars = new char[asciiEncoding.GetCharCount(rawData, 0, len)];

            asciiEncoding.GetChars(rawData, 0, len, chars, 0);
            return new(chars);
        }
        ...
        {
            // first 4 bytes is the string length, followed by the string.
            int len = reader.ReadInt32() * 2; // 2 byte chars
            byte[] rawData = reader.ReadBytes(len);
        ...
        {
            (int indexOf, int valueOffset, int nextOffset, byte[] valueAsByteArray, int valueAsInt) found = ReadIntAfter(playerFileContent, keyToLookFor, offset);
        ...
        {
            (int indexOf, int valueOffset, int nextOffset, byte[] valueAsByteArray, float valueAsFloat) found = ReadFloatAfter(playerFileContent, keyToLookFor, offset);
            if (found.indexOf != -1)
            {
                byte[] newValueBytes = BitConverter.GetBytes(newValue);
        ...
        private static readonly byte[] s_empty = [];
        ...
        {
            (int indexOf, int nextOffset) idx = BinaryFindKey(playerFileContent, keyToLookFor, offset);
            byte[] value = s_empty;
        ...
        public (int indexOf, int valueOffset, int nextOffset, int valueLen, byte[] valueAsByteArray, string valueAsString) ReadCStringAfter(byte[] playerFileContent, string keyToLookFor, int offset = 0)
        {
            ...
            if (idx.indexOf == -1)// Not found
                return (idx.indexOf, idx.nextOffset, idx.nextOffset, 0, s_empty, "");

            int len = BitConverter.ToInt32(new ArraySegment<byte>(playerFileContent, idx.nextOffset, sizeof(int)).ToArray(), 0);
            byte[] stringArray = new ArraySegment<byte>(playerFileContent, idx.nextOffset + sizeof(int), len).ToArray();
        ...
        public (int indexOf, int valueOffset, int nextOffset, int valueLen, byte[] valueAsByteArray, string valueAsString) ReadUnicodeStringAfter(byte[] playerFileContent, string keyToLookFor, int offset = 0)
        {
            ...
            int len = BitConverter.ToInt32(new ArraySegment<byte>(playerFileContent, idx.nextOffset, sizeof(int)).ToArray(), 0);
            byte[] stringArray = new ArraySegment<byte>(playerFileContent, idx.nextOffset + sizeof(int), len * 2).ToArray();
        ...
        public (int indexOf, int nextOffset) BinaryFindKey(byte[] dataSource, string key, int offset = 0)
        {
            ...
            byte[] keyWithLen = [.. BitConverter.GetBytes(key.Length), .. Encoding1252.GetBytes(key)];

            (int indexOf, int nextOffset) result = BinaryFindKey(dataSource, keyWithLen, offset);
        ...
        {
            int level = 0;
            string keybegin_block = "begin_block";
            string keyend_block = "end_block";
            (int, int, int, byte[], int) noMatch = (-1, 0, 0, [], 0);

            (int indexOf, int nextOffset) startPoint = BinaryFindKey(playerFileContent, keyToLookFor, offset);
        ...
        recurse:
            ...
            (int indexOf, int valueOffset, int nextOffset, byte[] valueAsByteArray, int valueAsInt) nextend_block = ReadIntAfter(playerFileContent, keyend_block, offset);
            ...
            (int indexOf, int valueOffset, int nextOffset, byte[] valueAsByteArray, int valueAsInt) nextbegin_block = ReadIntAfter(playerFileContent, keybegin_block, offset);
        ...
        {
            (int indexOf, int valueOffset, int nextOffset, int valueLen, byte[] valueAsByteArray, string valueAsString) found = ReadCStringAfter(playerFileContent, keyToLookFor, offset);
            if (found.indexOf == -1)
                return false;

            // Remove CString value
            byte[] keyBytes = Encoding1252.GetBytes(keyToLookFor);
            playerFileContent = [.. new[] {
                [.. playerFileContent.Take(found.indexOf - sizeof(int))], // start content
                BitConverter.GetBytes(keyToLookFor.Length),// KeyLen
                keyBytes,// Key
                BitConverter.GetBytes(0),// ValueLen
                [.. playerFileContent.Skip(found.nextOffset)],// end content
            }.SelectMany(a => a)];
        ...
        {
            (int indexOf, int valueOffset, int nextOffset, int valueLen, byte[] valueAsByteArray, string valueAsString) found = ReadUnicodeStringAfter(playerFileContent, keyToLookFor, offset);
            if (found.indexOf == -1)
                return false;
