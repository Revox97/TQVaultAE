...
namespace TQVaultAE.IO.Parsers
{
    ...
    public class DBRecordCollection(RecordId id, string recordType) : IEnumerable<Variable>
    {
		...
		public Variable? this[string variableName] => _variables.TryGetValue(variableName.ToUpperInvariant(), out Variable? val) ? val : null;
		...
		public string ToShortString() => string.Format(CultureInfo.CurrentCulture, "{0},{1},{2}", Id, RecordType, Count);
        ...
        public int GetInt32(string variableName, int index) => _variables.TryGetValue(variableName.ToUpperInvariant(), out Variable? val) ? val.GetInt32(index) : 0;
		...
		public float GetSingle(string variableName, int index) => _variables.TryGetValue(variableName.ToUpperInvariant(), out Variable? val) ? val.GetSingle(index) : 0.0F;
        ...
        public string GetString(string variableName, int index)
            => _variables.TryGetValue(variableName.ToUpperInvariant(), out Variable? val) ? val.GetString(index) ?? string.Empty : string.Empty;
        ...
        private KeyValuePair<string, Variable>[] _relevantVariables = [];
		...
		public KeyValuePair<string, Variable>[] RelevantVars
		{
			get
			{
                ...
                {
                    _relevantVariables ??= [.. _variables.Where(v => v.Value.IsValueRelevant)];
                    return _relevantVariables;
                }
